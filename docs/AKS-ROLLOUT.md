...
### DataProtection key persistence (CRITICAL before scale-out)

ASP.NET Core DataProtection keys are currently stored in-memory, per pod. In a
multi-replica deployment, each pod generates its own key ring. This means:

- Email confirmation tokens generated by one pod cannot be verified by another.
- "Invalid token" errors during registration/email-confirm flows at `replicaCount > 1`.

**Mitigation until Task 10.3 lands:**

Keep `replicaCount: 1` in the environment values file being deployed. The dev values file
already sets this. For prod, add:

```yaml
# values.prod.yaml — temporary override until DataProtection is wired
replicaCount: 1
hpa:
  minReplicas: 1
  maxReplicas: 1
...
Remove once Task 10.3 wires key persistence to Azure Blob or Azure SQL.
