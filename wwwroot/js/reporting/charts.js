...
(function () {
    ...
    // data: [{ Label, Value }, ...] or [{ label, value }, ...] (camelCase from IJsonHelper.Serialize)
    function getLabel(d) { return d.Label !== undefined ? d.Label : d.label; }
    function getValue(d) { return d.Value !== undefined ? d.Value : d.value; }
    ...
    function barChart(selector, data, opts) {
        ...
        var x = d3.scaleBand().domain(data.map(function (d) { return getLabel(d); }))
            .range([m.left, width - m.right]).padding(0.25);
        var y = d3.scaleLinear().domain([0, d3.max(data, function (d) { return getValue(d); }) || 1]).nice()
        ...
        svg.selectAll("rect.bar").data(data).enter().append("rect").attr("class", "bar")
            .attr("x", function (d) { return x(getLabel(d)); })
            .attr("y", function (d) { return y(getValue(d)); })
            .attr("width", x.bandwidth())
            .attr("height", function (d) { return (height - m.bottom) - y(getValue(d)); })
            .attr("rx", 3)
        ...
        svg.selectAll("text.val").data(data).enter().append("text").attr("class", "val")
            .attr("x", function (d) { return x(getLabel(d)) + x.bandwidth() / 2; })
            .attr("y", function (d) { return y(getValue(d)) - 4; })
    ...
    function donutChart(selector, data, opts) {
        ...
        var pie = d3.pie().value(function (d) { return getValue(d); }).sort(null);
        ...
        data.forEach(function (d, i) {
            ...
            item.append("span").text(getLabel(d) + " (" + getValue(d) + ")");
        });
    }
